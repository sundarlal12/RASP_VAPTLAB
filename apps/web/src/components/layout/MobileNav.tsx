...
export function MobileNav() {
  ...
  // The header has backdrop-blur, which — like transform or filter — makes
  // it the containing block for any `position: fixed` descendant instead of
  // the viewport. That collapsed this panel's top/bottom-derived height down
  // to the header's own ~65px instead of the full screen. Portaling straight
  // into <body> escapes the header entirely so `fixed` resolves against the
  // viewport again.
  const panel = open ? (
    <div className="fixed inset-x-0 top-[65px] bottom-0 z-[60] overflow-y-auto bg-white px-6 py-6">
